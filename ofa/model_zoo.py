...
def ofa_net(net_id, pretrained=True):
	if net_id == 'ofa_proxyless_d234_e346_k357_w1.3':
		...
		net = OFAResNets(
			...
			dropout_rate=0, depth_list=[0, 1, 2], expand_ratio_list=[0.15, 0.2, 0.25, 0.35], width_mult_list=[0.65, 0.8, 1.0]
		)
	...
	if pretrained:
		if net_id == 'ofa_resnet50_d=0+1+2_e=0.15+0.2+0.25+0.35_w=0.65+0.8+1.0':
			init = torch.load('./exp/kernel_depth2expand/phase1/checkpoint/model_best.pth.tar')['state_dict']
			net.load_state_dict(init)
